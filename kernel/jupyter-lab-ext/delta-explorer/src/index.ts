...
// Database Search Material Icon SVG (inlined)
const searchIcon = new LabIcon({
  name: 'delta-explorer:search',
  svgstr: `
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (app: JupyterFrontEnd) => {
    ...
    content.url = '/delta/';
    content.id = 'delta-explorer-iframe';
    content.title.label = 'Delta Explorer';
    content.title.closable = true;
    content.addClass('delta-explorer-iframe');
    content.sandbox = ['allow-scripts', 'allow-same-origin', 'allow-forms', 'allow-popups'];

    // Wrap the iframe in a widget for the sidebar
    const widget = new Widget();
    widget.id = 'delta-explorer-panel';
    widget.title.icon = searchIcon;
    widget.title.caption = 'Delta Explorer';
    widget.addClass('delta-explorer-panel');

    const layout = new PanelLayout();
    layout.addWidget(content);
    widget.layout = layout;

    // Add to the left sidebar at the top
    app.shell.add(widget, 'left', { rank: 0 });
  }
};
